...
def load_recipe_file(recipe_path: str) -> Dict[str, Any]:
    ...
    if not isinstance(recipe, dict):
        raise ValueError(
            f"Recipe file must contain a YAML dictionary, got: {type(recipe)}")
...
def find_recipe_files() -> list[Path]:
    ...
    recipe_files = list(recipes_dir.glob('*.yaml')) + list(
        recipes_dir.glob('*.yml'))
    return recipe_files


def find_all_matching_recipes(
        scenario: Dict[str, Any]) -> list[tuple[Path, Dict[str, Any]]]:
    ...
    for recipe_path in recipe_files:
        try:
            ...
            if all(
                    ...
                    for key in match_keys if key in scenario):
...
def compute_from_scenario(scenario: Dict[str, Any],
...
def merge_overrides(config: Dict[str, Any],
