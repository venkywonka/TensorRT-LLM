...
def validate_scenario(scenario: Dict[str, Any],
                      strict: bool = True) -> List[ScenarioValidationWarning]:
    ...
    missing_fields = [
        field for field in required_fields if field not in scenario
    ]
...
def validate_config(config: Dict[str, Any]) -> List[ScenarioValidationWarning]:
    ...
    if 'kv_cache_config' in config:
        ...
        if mem_frac is not None:
            if not isinstance(mem_frac,
                              (int, float)) or mem_frac <= 0 or mem_frac > 1:
                warnings.append(
                    ScenarioValidationWarning(
