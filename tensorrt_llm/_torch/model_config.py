...
class ModelConfig(Generic[TConfig]):
    ...
    def get_bindings_model_config(self,
        ...
        if hasattr(self.pretrained_config, 'num_kv_heads_per_layer'):
            ...
            if self.lora_config is not None and not all(kv == num_kv_heads_raw for kv in self.pretrained_config.num_kv_heads_per_layer):
                raise ValueError(
            ...
            num_kv_heads_raw = getattr(self.pretrained_config, 'num_key_value_heads',
                                       getattr(self.pretrained_config, 'num_query_groups',
                                               self.pretrained_config.num_attention_heads))

        num_kv_heads = num_kv_heads_raw // (self.mapping.tp_size * self.mapping.cp_size)
