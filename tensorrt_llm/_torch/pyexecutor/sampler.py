...
class EarlyStopWithMMResult(Sampler[SampleStateWithMMResult]):
    ...
    def update_requests(
        ...
        if len(mm_embeddings) != len(requests):
            raise ValueError(
                f"mm_embeddings count mismatch: {len(mm_embeddings)} != {len(requests)}"
            )
        for i, (request, mm_embedding) in enumerate(zip(requests, mm_embeddings)):
            request.state = LlmRequestState.GENERATION_COMPLETE
            # NOTE: This is a hack: set finish reason manually and set the beam 0
            request.set_finished_reason(FinishReason.LENGTH, 0)
            encoder_output_lengths = get_mm_encoder_output_lengths(request)
            if encoder_output_lengths is None:
                raise ValueError(
                    "multimodal encoder-output lengths are required to "
                    "package multimodal embedding handles"
                )
            if len(mm_embedding) != sum(encoder_output_lengths):
                raise ValueError(
                    f"mm_embedding shape mismatch: {len(mm_embedding)} != {sum(encoder_output_lengths)}"
                )

            request.py_result.append_mm_embeddings(mm_embedding, encoder_output_lengths)
