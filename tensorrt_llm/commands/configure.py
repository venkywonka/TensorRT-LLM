...
from tensorrt_llm.recipes import (compute_from_scenario, detect_profile,
                                  match_recipe, validate_config,
                                  validate_scenario)
...
def generate_serve_command(scenario: Dict[str, Any], cli_args: Dict[str, Any],
                           env: Dict[str, str], config_path: str) -> str:
...
def print_result(scenario: Dict[str, Any], config: Dict[str, Any],
                 env: Dict[str, str], cli_args: Dict[str, Any],
                 output_path: str, profile_name: str) -> None:
    ...
    click.echo(
        click.style(
            f"\nFound optimized configuration for the specified scenario:",
            fg='green',
            bold=True))
...
@click.option("--gpu",
              type=str,
              default=None,
              help="GPU type (e.g., 'H100_SXM', 'B200')")
@click.option("--num-gpus",
              type=int,
              default=None,
              help="Number of GPUs to use")
@click.option("--target-isl",
              type=int,
              default=None,
              help="Target input sequence length")
@click.option("--target-osl",
              type=int,
              default=None,
              help="Target output sequence length")
@click.option("--target-concurrency",
              type=int,
              default=None,
...
@click.option("--no-validate",
              is_flag=True,
              default=False,
              help="Skip validation of scenario constraints")
def configure(model: Optional[str], gpu: Optional[str], num_gpus: Optional[int],
              target_isl: Optional[int], target_osl: Optional[int],
              target_concurrency: Optional[int], tp_size: Optional[int],
              ep_size: Optional[int], profile: Optional[str],
              recipe: Optional[str], output: str, no_validate: bool):
    """Generate optimized TensorRT-LLM configuration from scenario constraints.
    ...
    try:
        ...
        if recipe:
            ...
            if config_from_recipe:
                ...
                profile_name = profile or scenario.get(
                    'profile') or detect_profile(scenario.get('model', ''))
                if profile_name:
        ...
        profile_name = profile or scenario.get('profile') or detect_profile(
            scenario.get('model', ''))
        if not profile_name:
            profile_name = "custom"

        # Print result
        print_result(scenario, config, env, cli_args, str(output_path),
                     profile_name)
