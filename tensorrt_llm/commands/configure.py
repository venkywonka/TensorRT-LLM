...
def print_result(scenario: Dict[str, Any], config: Dict[str, Any],
                 env: Dict[str,
                           str], output_path: str, profile_name: str) -> None:
    ...
    """
    click.echo(
        click.style(f"\nGenerated optimized recipe for the specified scenario:",
                    fg='green',
                    bold=True))
    ...
    # Print bench command
    click.echo(
        click.style("To run benchmarks with this recipe, use:",
                    fg='yellow',
                    bold=True))
    click.echo()
...
def configure(model: str, gpu: Optional[str], num_gpus: Optional[int],
              target_isl: int, target_osl: int, target_concurrency: int,
              tp_size: Optional[int], ep_size: Optional[int],
              profile: Optional[str], output: str, no_validate: bool):
    """Generate optimized TensorRT-LLM recipe from scenario constraints.
    ...
    try:
        ...
        if matched_recipe:
            click.echo(
                click.style("Found matching recipe in database!", fg='green'))
