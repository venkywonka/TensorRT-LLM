...
from tensorrt_llm.recipes import (find_all_matching_recipes, validate_config,
                                  validate_scenario)
...
def configure(model: str, gpu: Optional[str], num_gpus: Optional[int],
              ...
              tp_size: Optional[int], ep_size: Optional[int], output: str,
              no_validate: bool):
    ...
    try:
        ...
        click.echo(
            click.style(f"Found matching recipe: {recipe_path.name}",
                        fg='green'))
        ...
        # Print result
        print_result(matched_scenario, config, env, str(output_path),
                     profile_name)
