...
class TestRequireMmSpansIfNeeded:
    ...
    def test_no_mm_payload_never_raises(self):
        ...
        # Full prefill
        require_mm_spans_if_needed(None, begin_compute=0, end_compute=100,
                                   prompt_len=100)
        # Chunked
        require_mm_spans_if_needed(None, begin_compute=0, end_compute=50,
                                   prompt_len=100)
        # KV reuse
        require_mm_spans_if_needed(None, begin_compute=50, end_compute=100,
                                   prompt_len=100)
    ...
    def test_mm_payload_spans_present_partial_no_raise(self):
        ...
        require_mm_spans_if_needed(data, begin_compute=5, end_compute=50,
                                   prompt_len=100)

    def test_mm_payload_spans_present_full_no_raise(self):
        """Case 3: MM payload + spans + full prefill → no-op."""
        data = {"image": {}, "mm_contiguous_spans": [(10, 20)]}
        require_mm_spans_if_needed(data, begin_compute=0, end_compute=100,
                                   prompt_len=100)
    ...
    def test_mm_payload_spans_missing_full_warns(self):
        ...
        assert any("mm_contiguous_spans missing" in r.getMessage()
                   for r in records)
    ...
    def test_mm_payload_spans_missing_kv_reuse_raises(self):
        ...
        with pytest.raises(ValueError,
                           match="partial iteration.*begin_compute=20"):
            require_mm_spans_if_needed(data, begin_compute=20,
                                       end_compute=100, prompt_len=100)

    def test_mm_payload_spans_missing_chunked_raises(self):
        """Case 6: MM payload + no spans + end_compute < prompt_len → raise."""
        data = {"image": {"pixel_values": torch.zeros(1, 1)}}
        with pytest.raises(ValueError,
                           match="partial iteration.*end_compute=50"):
            require_mm_spans_if_needed(data, begin_compute=0,
                                       end_compute=50, prompt_len=100)
    ...
    def test_metadata_only_payload_never_raises(self):
        ...
        require_mm_spans_if_needed(data, begin_compute=0, end_compute=100,
                                   prompt_len=100)
        require_mm_spans_if_needed(data, begin_compute=0, end_compute=50,
                                   prompt_len=100)
        require_mm_spans_if_needed(data, begin_compute=20, end_compute=100,
                                   prompt_len=100)
    ...
    def test_warning_dedup(self):
        ...
        span_warnings = [r for r in records
                         if "mm_contiguous_spans missing" in r.getMessage()]
